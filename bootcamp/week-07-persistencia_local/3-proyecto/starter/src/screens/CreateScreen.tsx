    ...
    createItem(
      ...
      },
      {
        onSuccess: () => {
          Alert.alert(
            '¡Proyecto Registrado!',
            'La nueva vivienda fue guardada e indexada en la caché local y servidor.',
        ...
        onError: (error) => {
          Alert.alert(
            'Error al guardar',
            error.message || 'Ocurrió un error al registrar el proyecto.'
          );
        },
      }
    );
      ...
      >
        <View style={styles.introBox}>
          ...
          <Text style={styles.introText}>
            Registra una nueva opción habitacional para asociados con persistencia
            automática en catálogo y almacenamiento local.
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  tipoContainer: { gap: SPACING.xs },
  tipoLabel: { ...TYPOGRAPHY.label, textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  tipoOptionText: { fontSize: 13, fontWeight: '600', color: COLORS.textMuted },
  tipoOptionTextSelected: { color: '#FFFFFF', fontWeight: '700' },
  errorText: { ...TYPOGRAPHY.error, marginTop: 2 },
  switchRow: {
    flexDirection: 'row',
  ...
  switchTextContainer: { flex: 1, paddingRight: SPACING.md, gap: 2 },
  switchTitle: { ...TYPOGRAPHY.body, fontWeight: '600', color: COLORS.text },
  switchSubtitle: { ...TYPOGRAPHY.caption, color: COLORS.textMuted },
